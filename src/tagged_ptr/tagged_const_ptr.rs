...
pub struct TaggedConstPtr<T> {
    ptr: usize,
    _marker: PhantomData<*const T>,
}

impl<T> TaggedConstPtr<T> {
    /// Creates a new tagged pointer from a normal one with the tag value set to 0.
    pub fn new_zero(value: *const T) -> Self {
        Self {
            ptr: value as usize,
            _marker: PhantomData,
        }
    ...
    pub fn new(value: *const T, tag: usize) -> Self {
    ...
    #[inline]
    #[must_use]
    pub const fn original(&self) -> *const T {
        self.clean() as *const T
    }
...
mod tests {
    ...
    fn test() {
        ...
        let mut f = TaggedConstPtr::new_zero(f);
        assert_eq!(f.max(), 3);
